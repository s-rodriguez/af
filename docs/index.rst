 ...
 * :class:`af.model.algorithms.BaseAlgorithm` class, so to be able to implement new anonymization algorithms for existing or new privacy models. (3 algorithms provided with the library: Datafly, Incognito-K and Incognito-L)
 * :class:`af.model.hierarchies.BaseHierarchy` class built with the idea of being able to generalize/supress any type of attribute (string, int, etc...).
